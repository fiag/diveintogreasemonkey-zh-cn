...
<!-- ********************************************************************
     $Id: autotoc.xsl,v 1.3 2005/05/01 18:32:38 f8dy Exp $
     ********************************************************************
  ...
  <xsl:element name="{$toc.listitem.type}">
    <span class='refentrytitle'>
      <a>
        <xsl:attribute name="href">
          <xsl:call-template name="href.target">
            <xsl:with-param name="context" select="$toc-context"/> <!-- MAP fix path bug if refentry is in different directory than TOC -->
          </xsl:call-template>
        </xsl:attribute>
